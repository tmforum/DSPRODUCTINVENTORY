...
import org.tmf.dsmapi.commons.utils.WorkflowValidator;
...
public class ProductResource {
    ...
    public Response patch(@PathParam("id") long id, Product partialProduct) throws BadUsageException, UnknownResourceException {
        Response response = null;
        Product currentProduct = productInventoryFacade.find(id);
        ...
        if (currentProduct != null) {
            ...
            if (!BeanUtils.verify(partialProduct, node, token)) {
                ...
                throw new BadUsageException(ExceptionType.BAD_USAGE_MANDATORY_FIELDS, "current=" + currentProduct.getStatus() + " sent=" + partialProduct.getStatus());
            }

            partialProduct.setId(id);
            if (WorkflowValidator.isCorrect(currentProduct.getStatus(), partialProduct.getStatus())) {
                BeanUtils.patch(currentProduct, partialProduct, node);
                publisher.valueChangedNotification(currentProduct, new Date());
            } else {
                throw new BadUsageException(ExceptionType.BAD_USAGE_STATUS_TRANSITION, "current=" + currentProduct.getStatus() + " sent=" + partialProduct.getStatus());
            }
            // 201 OK + location
